"""Artifact listing/download handler with in-memory stub."""

from datetime import datetime, timezone
from uuid import uuid4

from fastapi import HTTPException

from backend.schemas.artifacts import ArtifactResponse
from backend.security.auth import EnterpriseContext

_artifacts: dict[str, dict] = {}


async def list_artifacts(
    enterprise: EnterpriseContext,
    thread_id: str | None = None,
    limit: int = 50,
    offset: int = 0,
) -> list[ArtifactResponse]:
    results = [
        d
        for d in _artifacts.values()
        if d["enterprise_id"] == enterprise.enterprise_id
        and (thread_id is None or d.get("thread_id") == thread_id)
    ]
    results.sort(key=lambda x: x["created_at"], reverse=True)
    return [ArtifactResponse(**d) for d in results[offset : offset + limit]]


async def get_artifact(
    artifact_id: str, enterprise: EnterpriseContext
) -> ArtifactResponse:
    d = _artifacts.get(artifact_id)
    if not d or d["enterprise_id"] != enterprise.enterprise_id:
        raise HTTPException(status_code=404, detail="Artifact not found")
    return ArtifactResponse(**d)


async def download_artifact(
    artifact_id: str, enterprise: EnterpriseContext
) -> bytes:
    """Returns file bytes. Stub returns placeholder text."""
    d = _artifacts.get(artifact_id)
    if not d or d["enterprise_id"] != enterprise.enterprise_id:
        raise HTTPException(status_code=404, detail="Artifact not found")
    return b"Placeholder artifact content. Agent execution not wired yet."
