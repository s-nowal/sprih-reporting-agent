...
def _register_output_artifacts(
    ...
    """Scan output/ after a run and upsert artifact records in the in-memory store.

    Removes any stale artifacts for this thread first so re-runs don't
    accumulate duplicates, then registers every file found under output/.

    Args:
        temp_workspace: The agent workspace root (contains output/ subdir).
        enterprise_id: Tenant identifier, used for ownership and S3 path.
        thread_id: Conversation thread identifier.
    """
    # Remove stale artifacts for this thread before re-registering
    for k in [k for k, v in artifact_handler._artifacts.items() if v["thread_id"] == thread_id]:
        del artifact_handler._artifacts[k]
    ...
    for file_path in output_dir.rglob("*"):
        ...
        rel = file_path.relative_to(temp_workspace)
        s3_path = f"enterprise/{enterprise_id}/workspaces/{thread_id}/{rel.as_posix()}"
        artifact_handler._artifacts[artifact_id] = {
            "id": artifact_id,
            "thread_id": thread_id,
...
async def stream_run(
    ...
    *,
    ...
    temp_workspace = base_dir / "agent_folder"
    ...
    try:
        ...
        _register_output_artifacts(temp_workspace, enterprise.enterprise_id, thread_id)
